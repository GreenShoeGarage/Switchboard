/*
 * SWITCHBOARD Agent v0.8.0-device-workbench-candidate
...
constexpr char AGENT_VERSION[] = "0.8.0-device-workbench-candidate";
...
enum class ManagedMode : uint8_t { INPUT_MODE, INPUT_PULLUP_MODE, OUTPUT_MODE, ANALOG_MODE, PWM_MODE, DAC_MODE };
...
const char *modeName(ManagedMode mode) {
  switch (mode) {
    ...
    case ManagedMode::DAC_MODE: return "DAC";
  }
  return "INPUT";
}
...
bool supportsPwm(const char *pinId) {
  return strcmp(pinId, "D3") == 0 || strcmp(pinId, "D5") == 0 || strcmp(pinId, "D6") == 0 ||
    strcmp(pinId, "D9") == 0 || strcmp(pinId, "D10") == 0 || strcmp(pinId, "D11") == 0;
}

bool supportsAnalog(const char *pinId) {
  return pinId[0] == 'A' && pinId[1] >= '0' && pinId[1] <= '5' && pinId[2] == '\0';
}

bool supportsDac(const char *pinId) {
  return strcmp(pinId, "A0") == 0;
}

int samplePin(ManagedPin &pin) {
  if (pin.mode == ManagedMode::ANALOG_MODE) return analogRead(pin.hardwarePin);
  if (pin.mode == ManagedMode::PWM_MODE || pin.mode == ManagedMode::DAC_MODE) return pin.value;
...
void sendHeartbeat() {
  ...
  JsonObject localFailsafe = message.createNestedObject("localFailsafe");
  localFailsafe["mode"] = "SAFE_INPUT_BOOT";
  localFailsafe["timeoutMs"] = nullptr;
  String payload;
  serializeJson(message, payload);
  webSocket.sendTXT(payload);
}
...
void applyGpioCommand(JsonDocument &message) {
  ...
  if (strcmp(kind, "SET_MODE") == 0) {
    ...
    } else if (strcmp(requestedMode, "ANALOG") == 0 && supportsAnalog(pin->id)) {
      pin->mode = ManagedMode::ANALOG_MODE;
    } else if (strcmp(requestedMode, "PWM") == 0 && supportsPwm(pin->id)) {
      analogWrite(pin->hardwarePin, 0);
      pin->mode = ManagedMode::PWM_MODE;
      pin->value = 0;
    } else if (strcmp(requestedMode, "DAC") == 0 && supportsDac(pin->id)) {
      analogWrite(pin->hardwarePin, 0);
      pin->mode = ManagedMode::DAC_MODE;
      pin->value = 0;
    } else {
      sendAcknowledgment(commandId, pin, "UNSUPPORTED_MODE");
      logEvent("WARN", "GPIO_MODE_REJECTED", "Mode is not supported by this physical pin");
      return;
    }
    pin->value = samplePin(*pin);
    snapshotDirty = true;
  ...
  if (strcmp(kind, "WRITE_PWM") == 0 || strcmp(kind, "WRITE_DAC") == 0) {
    const bool pwmCommand = strcmp(kind, "WRITE_PWM") == 0;
    const bool modeMatches = pwmCommand ? pin->mode == ManagedMode::PWM_MODE : pin->mode == ManagedMode::DAC_MODE;
    if (!modeMatches || (pwmCommand ? !supportsPwm(pin->id) : !supportsDac(pin->id))) {
      sendAcknowledgment(commandId, pin, "OUTPUT_MODE_MISMATCH");
      logEvent("WARN", "ANALOG_OUTPUT_REJECTED", "Pin mode or capability did not match command");
      return;
    }
    const int requestedValue = message["requestedValue"] | -1;
    if (requestedValue < 0 || requestedValue > 4095) {
      sendAcknowledgment(commandId, pin, "INVALID_12_BIT_VALUE");
      return;
    }
    analogWrite(pin->hardwarePin, requestedValue);
    pin->value = requestedValue;
    snapshotDirty = true;
    sendAcknowledgment(commandId, pin);
    logEvent("INFO", pwmCommand ? "PWM_WRITE_APPLIED" : "DAC_WRITE_APPLIED", pin->id);
    return;
  }
...
void handleServerMessage(uint8_t *payload, size_t length) {
  ...
  if (strcmp(type, "device.snapshot.ack") == 0) {
    snapshotAccepted = true;
    logEvent("INFO", "SNAPSHOT_SYNCHRONIZED", "Full state accepted; command polling enabled");
    pollForCommand();
    return;
  }
...
void processProvisioningLine(const String &line) {
  ...
  if (strcmp(action, "status") == 0 || strcmp(action, "identify") == 0) {
    ...
    status["localFailsafeMode"] = "SAFE_INPUT_BOOT";
    status["localFailsafeTimeoutMs"] = nullptr;
...
void loop() {
  ...
  if (snapshotAccepted && now - lastCommandPollAt >= COMMAND_POLL_INTERVAL_MS) {
